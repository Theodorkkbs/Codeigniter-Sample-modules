...
public function get_projects_info($project_id){
	...
	$query_get_data=$this->db->get('projects');
	return $query_get_data->row();
...
$this->db->select('
	...
	'); //select from tasks the task name etc etc from projects the project name etc etc
$this->db->from('tasks');
$this->db->join('projects','projects.id = tasks.project_id'); //join projects with projects id = tasks.projects_id
$this->db->where('tasks.project_id',$project_id);   

if($active == true){
	$this->db->where('tasks.status',0);
}else{
		$query=$this->db->where('tasks.status',1);
}
$query=$this->db->get();
